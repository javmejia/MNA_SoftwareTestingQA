...
from src.exceptions import DuplicateEntityError, PersistenceError
from src.models import Customer, Hotel, Reservation
from src.repositories.base_json_repository import BaseJsonRepository
...
class TestBaseJsonRepository(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.file_path = Path(self.temp_dir.name) / "hotels.json"
        self.repo = BaseJsonRepository(self.file_path, Hotel.from_dict, "hotel")
    ...
    def test_load_all_invalid_json_prints_error_and_returns_empty(self) -> None:
        self.file_path.write_text("{", encoding="utf-8")
        with patch("builtins.print") as mock_print:
            hotels = self.repo.load_all()
        self.assertEqual(hotels, [])
        self.assertTrue(any("[DATA ERROR]" in str(call) for call in mock_print.call_args_list))

    def test_load_all_non_array_root_prints_error_and_returns_empty(self) -> None:
    ...
    def test_load_all_missing_file_prints_warning_and_returns_empty(self) -> None:
        self.file_path.unlink()
        with patch("builtins.print") as mock_print:
            hotels = self.repo.load_all()
        self.assertEqual(hotels, [])
        self.assertTrue(any("[DATA WARNING]" in str(call) for call in mock_print.call_args_list))
...
class TestEntityRepositories(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.reservation_repo = ReservationRepository(base / "reservations.json")
    ...
    def test_customer_repository_crud(self) -> None:
        ...
        self.assertEqual(self.customer_repo.get_customer("C001").full_name, "Jane Doe")
    ...
    def test_reservation_repository_crud_and_cancel_paths(self) -> None:
        ...
        self.assertEqual(self.reservation_repo.get_reservation("R001").status, "active")
